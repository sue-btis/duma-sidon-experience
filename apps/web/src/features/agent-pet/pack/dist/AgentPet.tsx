...
export function AgentPet({
  ...
  return (
    <div className={`agent-pet agent-pet--${state} ${className}`} role="img" aria-label={label ?? `${petManifest.pet.name}: ${state}`}>
      ...
      {path && <svg className="agent-pet__orbit-svg agent-pet__orbit-svg--front" viewBox="0 0 192 192" aria-hidden="true"><defs><clipPath id="pet-orbit-front"><rect y={centerY} width="192" height="88" /></clipPath></defs><path d={orbitPath(path)} clipPath="url(#pet-orbit-front)" />{orbits.map((orbit, index) => <g key={`${orbit.name ?? "orbit"}-${index}`} clipPath="url(#pet-orbit-front)"><circle r={(orbit.size_px ?? 12) / 2}><animateMotion dur={`${orbit.duration_ms ?? 3600}ms`} begin={`${orbit.delay_ms ?? 0}ms`} repeatCount="indefinite" path={orbitPath(orbit)} /></circle></g>)}</svg>}
