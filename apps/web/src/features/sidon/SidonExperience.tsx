...
export async function SidonExperience({ locale }: Readonly<{ locale: "es" | "en" }>) {
  ...
  return <main className={`${styles.page} ${styles.sidonLanding}`}>
    ...
    <section aria-label={t("categoriesTitle")} className={`${styles.scene} ${styles.categoriesScene}`}><SidonCategories categories={categories} instructions={t("categoryInstructions")} locale={locale} nextLabel={t("nextCategory")} previousLabel={t("previousCategory")} /><div className={styles.connected}><h3>{t("connectedTitle")}</h3><p>{t("connectedLead")}</p></div></section>
