...
export async function SidonExperience({ locale }: Readonly<{ locale: "es" | "en" }>) {
  ...
  return <main className={`${styles.page} ${styles.sidonLanding}`}>
    <section aria-labelledby="sidon-title" className={`${styles.scene} ${styles.intro}`}><LetterWorldsCanvas variant="digital" /><div className={styles.introIdentity}><Image alt="Sidón" height={104} priority src="/home/worlds/sidon.png" unoptimized width={104} /><h1 id="sidon-title">{t("headline")}</h1></div><div className={styles.introDescription}><span>{t("lead")}</span><p className={styles.introMeta}>{t("introMeta")}</p></div></section>
