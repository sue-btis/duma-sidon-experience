...
export async function SidonExperience({ locale }: Readonly<{ locale: "es" | "en" }>) {
  ...
  return <main className={`${styles.page} ${styles.sidonLanding}`}>
    <section aria-labelledby="sidon-title" className={`${styles.scene} ${styles.intro}`}><div aria-hidden="true" className={styles.orbits}><i /><i /></div><div><Image alt="Sidón" height={104} priority src="/home/worlds/sidon.png" unoptimized width={104} /><h1 id="sidon-title">{t("headline")}</h1><span>{t("lead")}</span><p className={styles.introMeta}>{t("introMeta")}</p></div></section>
    <section aria-label={t("categoriesTitle")} className={`${styles.scene} ${styles.categoriesScene}`}><SidonCategories categories={categories} instructions={t("categoryInstructions")} locale={locale} modulesLabel={t("modules")} nextLabel={t("nextCategory")} previousLabel={t("previousCategory")} /><div className={styles.connected}><h3>{t("connectedTitle")}</h3><p>{t("connectedLead")}</p></div></section>
