...
export async function SidonExperience({ locale }: Readonly<{ locale: "es" | "en" }>) {
  ...
  return <main className={`${styles.page} ${styles.sidonLanding}`}>
    <section aria-labelledby="sidon-title" className={`${styles.scene} ${styles.intro}`}><TwoOrbitFigure className={styles.orbits} nodeColors={["var(--sidon)", "var(--sidon)"]} orbitColors={["var(--sidon)", "var(--sidon)"]} /><div><Image alt="Sidón" height={104} priority src="/home/worlds/sidon.png" unoptimized width={104} /><h1 id="sidon-title">{t("headline")}</h1><span>{t("lead")}</span><p className={styles.introMeta}>{t("introMeta")}</p></div></section>
