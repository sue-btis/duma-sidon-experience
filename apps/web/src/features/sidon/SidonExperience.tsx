...
export async function SidonExperience({ locale }: Readonly<{ locale: "es" | "en" }>) {
  ...
  return <main className={`${styles.page} ${styles.sidonLanding}`}>
    <section aria-labelledby="sidon-title" className={`${styles.scene} ${styles.intro}`}><WorldHero description={t("lead")} logo="/home/worlds/sidon.png" logoAlt="Sidón" logoHeight={104} logoWidth={104} meta={t("introMeta")} priority title={t("headline")} titleId="sidon-title" variant="digital" /></section>
    <section aria-label={t("categoriesTitle")} className={`${styles.scene} ${styles.categoriesScene}`}><SidonCategories categories={categories} closeLabel={t("closeCategory")} exploreLabel={t("explore", { category: "{category}" })} instructions={t("categoryInstructions")} locale={locale} modulesLabel={t("modules")} nextLabel={t("nextCategory")} previousLabel={t("previousCategory")} solvesLabel={t("whatItSolves")} /><div className={styles.connected}><h3>{t("connectedTitle")}</h3><p>{t("connectedLead")}</p></div></section>
