...
export async function DumaExperience({ locale }: Readonly<{ locale: "es" | "en" }>) {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.scene}><header><h2>{t("embeddedTitle")}</h2><p>{t("embeddedLead")}</p></header><ul className={styles.embedded}>{embedded.map((item) => <li key={item}>{item}</li>)}</ul></section>
    ...
    <section className={`${styles.scene} ${styles.close}`}><h2>{t("ctaTitle")}</h2><p>{t("ctaLead")}</p><Link aria-label={t("ctaTitle")} href={`/${locale}/sidon/conversemos/?source_path=%2Fsidon%2Fduma&world=sidon&interest=duma`}>{t("cta")}<ArrowRight aria-hidden="true" size={17} /></Link></section>
  </main>;
}
