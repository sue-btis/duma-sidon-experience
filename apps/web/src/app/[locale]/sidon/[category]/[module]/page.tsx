...
export default async function SidonModulePage({ params }: Props) {
  ...
  return <div className="bg-background"><div className="fixed inset-x-0 top-0 z-10 px-4 pt-5 sm:px-6"><EcosatNavbar locale={locale} page="sidon" path={`/sidon/${category.slug}/${moduleDefinition.slug}/`} /></div><SidonModuleExperience categoryName={categoryContent.name} categorySlug={category.slug} contactAction={t("conversationAction")} contactEyebrow={t("moduleContactEyebrow")} contactLead={t("conversationLead")} contactTitle={t("conversationTitle")} description={moduleContent.description} dumaGame={t.raw("dumaGame") as { invitationAction: string; invitationBody: string; invitationTitle: string }} locale={locale} moduleIcon={moduleDefinition.icon} moduleName={moduleContent.name} moduleSlug={moduleDefinition.slug} narrative={narratives[moduleDefinition.key]} solves={categoryContent.solves} /></div>;
}
